...
- **Image controls** — click any image in the document body to reveal a toolbar: drag the corner handles (or pick S / M / L / Full) to resize, Replace to swap it via the same drill-down picker (upload / browse / link / clipboard), Compress to re-encode it locally in the browser (lossless-ish canvas pass, no server round-trip, no quality slider needed for typical file-size reduction), Reveal in Finder to open the containing folder in the OS, and Remove to delete the node. Resized images are stored as a standard HTML `<img src="…" width="…">` so they render at the right size everywhere the file is read (GitHub, Obsidian, VS Code preview, other editors); images that have never been resized continue to round-trip as plain `![](src)` Markdown. The board image manager (table and kanban image cells) also gained per-thumbnail **Compress** and **Reveal in Finder** actions.
