...
- **See what changed before resolving a conflict** — the external-edit conflict banner now has a **"What changed ▾"** reveal that expands a row-aligned, side-by-side diff (your version vs. the version on disk) of just the changed lines — added rows in green, removed in red, changed in amber, with hatched gaps where a line exists on only one side. Choosing *Reload from disk* vs *Keep my version* is now an informed decision instead of a blind one. (A full diff viewer is planned; this leaves the hook for it.)
