...
- **Row handle + cell selection for regular tables (c46)** — plain markdown tables now have a Notion-style rail on their left edge. **Hover a row** → a `⠿` grip appears, attached to the table so it's actually reachable; **click** it to select the whole row and open a menu (**Insert row above / below**, **Duplicate row**, **Delete row**), or **drag** it to reorder. Tables also gained a real **cell-selection state**: the active cell is outlined, and you can **drag across cells / shift-click** to select a block. The header row stays first — its menu only offers **Insert row below**, and it can't be dragged, deleted, or pushed down, so the table always keeps a valid header. The grip only shows when the document is editable and never appears on board tables (which have their own). The margin block handle still moves the whole table. (c46)
