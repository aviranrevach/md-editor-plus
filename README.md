...
**Row & column handles + selection** — Notion-style edge handles. **Hover a cell** → a thin stroke appears on the left edge (that **row**'s handle) and the top edge (that **column**'s handle). **Click** a stroke to select the whole row/column and open its menu (rows: **Insert above / below**, **Duplicate**, **Delete**; columns: **Insert left / right**, **Duplicate**, **Delete**), or **drag** it to reorder. The active cell is outlined, and you can **drag across cells / shift-click** to select a block. The header row stays first (menu: **Insert row below** only), and a table always keeps at least one column. The margin handle still moves the whole table.
