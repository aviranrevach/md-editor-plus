...
| Add a Notion-style row handle to regular markdown tables (hover gutter + grip), with drag-to-reorder and a row-actions menu — the affordance c36 adds to board tables, but for plain tables (which today have no dragger at all). | Review | c46 | Medium | Tables |  |
